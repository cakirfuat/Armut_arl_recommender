...
data = pd.read_csv('.venv/lib/datasets/armut_data.csv')
data.head()
data.describe()
data.shape
...
df_pivot = df.groupby(['ID', 'Service'])['Service'].count().unstack().fillna(0).map(lambda x: 1 if x > 0 else 0)


# Adım 2: Birliktelik kurallarını oluşturunuz.
df_pivot = df_pivot.astype(bool)
frequent_itemsets = apriori(df_pivot, min_support=0.005, use_colnames=True)
...
arl_recommender(rules,"49_1",rec_count=2)